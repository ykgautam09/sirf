...
router.post(
  ...
  function (req, res) {
    ...
    db.connection
      .promise()
      .query("SELECT id FROM `institute` WHERE email=?;", userData.email)
      .then((data) => {
        ...
        db.connection
          .promise()
          .query("INSERT INTO `institute` SET ?;", userData)
          ...
          .then((user) => {
            ...
            db.connection
              .promise()
              .query("INSERT INTO `otp` SET ?", {
                otp,
                user_id: user[0].insertId
              })
              .then(() => {
                console.log(
                  "otp inserted in otp table for user",
                  userData.email
                );
                ...
                ejs
                  .renderFile(
                    ...
                    { verificationLink, email: userData.email }
                  )
                  .then((data) => {
...
router.get("/verify/:email", async function (req, res) {
  ...
  db.connection
    .promise()
    .query(
      `select otp from otp where user_id=(select id from institute where email=? LIMIT  1) and time > now() - INTERVAL ${durationAllowed} MINUTE ORDER BY id DESC LIMIT 1;`,
      email
    )
    .then((result) => {
      ...
      if (otp == pass) {
        ...
        db.connection
          .promise()
          .query(
            "DELETE FROM otp WHERE user_id=(SELECT id FROM institute WHERE email=? LIMIT 1) and otp=?;",
            [email, otp]
          )
          .then(() =>
            console.log("current otp removed from database for :", email)
          );
      }
    })
    ...
    .then(() => {
      ...
      db.connection
        .promise()
        .query("UPDATE institute SET password=? WHERE email=?;", [
          passHash,
          email
        ])
        .then(() => {
...
router.post("/login", function (req, res) {
  ...
  db.connection
    .promise()
    .query("SELECT `password` FROM `institute` WHERE `email`=? LIMIT 1", email)
    .then(async (result) => {
      ...
      if (cred.compareHash(password, hash)) {
        ...
        ejs
          .renderFile(
            path.join(__dirname, "..", "views", "Mail", "login.ejs"),
            { email, pass: password }
          )
          .then((content) => {
